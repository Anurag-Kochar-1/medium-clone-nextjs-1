            ...
            <div className='w-[70%]   h-auto flex flex-col items-start justify-start'>
                    ...
                    <p className='px-3 py-1  text-sm rounded-full bg-gray-200'> {blog.category} </p>
                        ...
                        <div className=' flex items-center space-x-3'>
                            ...
                            <BsDashCircle className='hidden sm:inline w-4 h-4 text-gray-700 hover:cursor-pointer hover:text-red-700  md:w-5 md:h-5'/>
                            {/* <BsThreeDots className='w-4 h-4 text-gray-700 hover:cursor-pointer'/> */}
                            {session?.user?.email === blog.userEmail &&  <Dropdown source="feed" blogId={blog._id} creatorEmail={blog.userEmail as string} />}
                        </div>
                    </div>
            </div>
